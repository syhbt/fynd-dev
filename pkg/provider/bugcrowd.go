...
func ListBugcrowdDomain() ([]string, error) {
	bugcrowdData, err := GetBugcrowdData()
	if err != nil {
		return nil, err
	}

	var bugcrowdDomain []string
	for _, data := range bugcrowdData {
		for _, target := range data.Targets.InScope {
			if target.AssetType == "website" {
				bugcrowdDomain = append(bugcrowdDomain, target.AssetIdentifier)
			}
		}
	}

	return bugcrowdDomain, nil
}
